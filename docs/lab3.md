...
!!! question
    :question:   
        Go to the OMIM page for this gene. What phenotype on OMIM might fit with this inheritance mode and clincical history? (**hint**: what phenotype has the same inheritance as our case and has similar phenotype)
...
#### Clinical and Family History
A patient diagnosed with colon cancer and with a family history of colon cancer. Genetic testing to determine if the patient has Lynch Syndrome (hereditary nonpolyposis colorectal cancer). The analysis of SNPs and Indel variants did not reveal any likely pathogenc variants. Therefore structural variant calling was performed using the structural variant (SV) caller Manta which combines paired and split-read evidence to call SVs. 

#### Variant filtration
Take a look inside the file called ```case4_manta.vep_annotated.vcf.gz```. You can see that this file is annotated with VEP and has information from ClinVar. We perform three filtration steps to narrow down the list of SVs: 
...
Use grep to extract variants that overlap with the genes listed above and view any detected regions in IGV. Using the fitltered vcf file answer the following questions:


!!! question
    :question: 
    Which gene(s) are impacted by structural variants?
...
!!! question
    :question: 
    Do they overlap with any know pathogenic variants of the same type?
